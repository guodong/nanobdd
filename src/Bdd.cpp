...
Bdd::Bdd() : root_(nodeTable->falseNode()) { root_->ref(); }

Bdd::Bdd(Node *root) : root_(root) { root_->ref(); }
...
Bdd::~Bdd() {
  if (root_ != nullptr && nodeTable != nullptr) {
    root_->deref();
  }
};
...
Node *Bdd::root() const { return root_; }

bool Bdd::isFalse() const { return root_ == nodeTable->falseNode(); }

bool Bdd::isTrue() const { return root_ == nodeTable->trueNode(); }
...
Bdd Bdd::operator!() const { return nodeTable->bddNot(root_); }
...
bool Bdd::operator==(const Bdd &r) const { return root_ == r.root(); }
